...
public interface PlatformQaReplyRepository extends JpaRepository<PlatformQaReply, Integer> {
    ...
    @Query(value = "SELECT A.PLATFORM_QA_NO, A.PLATFORM_QA_REPLY_NO, A.PLATFORM_QA_REPLY_CONTENT, A.ADMIN_NO, A.PLATFORM_QA_REPLY_REGIST_DATE, A.PLATFORM_QA_REPLY_STATUS FROM TBL_PLATFORM_QA_REPLY A ORDER BY A.PLATFORM_QA_NO ASC",
            nativeQuery = true)
